...
void chip8_load(struct chip8 *chip8, const char *buff, size_t size)
{
    ...
    memcpy(&chip8->memory.memory[CHIP8_PROGRAM_LOAD_ADDRESS], buff, size);
...
static void chip8_exec_extended_eight(struct chip8 *chip8, unsigned short opcode)
{
    ...
    // Set Vx = Vx + Vy - 0x8xy4 set VF = carry.
    case 0x04:
...
static void chip8_exec_extended_F(struct chip8 *chip8, unsigned short opcode)
{
    ...
    // fx0a - LD Vx, K. Wait for a press, store the value of the key in Vx
    case 0x0A:
    {
